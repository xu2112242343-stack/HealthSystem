...
export function DiseaseInternalRiskStructureChart({ data }: { data: RiskStructureRow[] }) {
  ...
  return (
    <div className="w-full">
      <p className="mb-5 text-center text-[11px] leading-relaxed text-slate-500 sm:text-xs">
        三枚环形成分图并列：环上角度 ∝ 人数；中心为该病种分层合计（与队列样本量一致）。
        <span className="hidden sm:inline"> 悬停扇区查看人数与占比。</span>
      </p>

      <div className="mb-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 border-b border-slate-100 pb-4 text-[11px] text-slate-600">
            ...
            >
              <div className="relative h-[220px] w-full sm:h-[200px]">
                ...
                <div className="pointer-events-none absolute left-1/2 top-1/2 z-10 w-[min(46%,7.5rem)] -translate-x-1/2 -translate-y-1/2 text-center">
                  <p className="text-[11px] font-semibold tracking-tight text-slate-700">{row.disease}</p>
                </div>
              </div>

              <div className="mt-3 flex w-full flex-wrap justify-center gap-x-2 gap-y-1.5 text-[10px]">
